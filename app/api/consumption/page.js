...
export default function Consumption() {
  ...
  useEffect(() => {
    fetch('/api/consumption').then(r => r.json()).then(d => setConsumption(d.data))
    fetch('/api/cpi').then(r => r.json()).then(d => setCpi(d))
  }, [])

  if (!consumption || !cpi?.headline?.length) return (
    <div style={{padding:'40px',fontFamily:'sans-serif',color:'#666'}}>Loading...</div>
  )

  const latest = consumption.at(-1)
  const prev = consumption.at(-2)
  const diff = latest && prev ? (latest.value - prev.value).toFixed(1) : '0.0'
  const diffStr = Number(diff) > 0 ? `+${diff}pp` : `${diff}pp`

  // CPI（headline）と消費支出の共通期間を合わせる
  const cpiMap = {}
  cpi.headline.forEach(v => { cpiMap[v.date] = v.value })
  const cpiCoreMap = {}
  cpi.core.forEach(v => { cpiCoreMap[v.date] = v.value })

  const labels = consumption.map(v => v.date)
  const cpiAligned = labels.map(d => cpiMap[d] ?? null)
  const cpiCoreAligned = labels.map(d => cpiCoreMap[d] ?? null)
    ...
    datasets: [
      {
        ...
        data: consumption.map(v => v.value),
        borderColor: '#9B59B6',
        backgroundColor: consumption.map(v => v.value >= 0 ? 'rgba(155,89,182,0.7)' : 'rgba(226,75,74,0.7)'),
        borderWidth: 1,
        type: 'bar',
      }
    ]
  ...
  const s = {
    ...
    card:      { background: '#f8f8f6', borderRadius: '10px', padding: '14px 16px', marginBottom: '20px' },
    ...
    <main style={s.wrap}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px', marginBottom: '20px' }}>
        ...
        <div style={s.card}>
          ...
          <div style={{ fontSize: '11px', color: '#888', marginTop: '3px' }}>Real consumption adjusted</div>
        </div>
      </div>
      ...
      <div style={s.grid2}>
        ...
        <div style={s.box}>
          ...
          <div style={s.note}>※ CPI data may be 1–2 months ahead of consumption</div>
        </div>
      </div>
    </main>
  )
